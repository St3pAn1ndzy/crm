...
class CrmBusinessLogicTestCase(TestCase):
    def setUp(self):
        ...
        self.service = Service.objects.create(title="Тестовая услуга", price=10000, is_active=True)
        ...
        self.lead_in_work = Lead.objects.create(
            first_name="Иван", last_name="ПростоЛид", status="new", advertisement=self.ad
        )
        self.lead_refused = Lead.objects.create(
            first_name="Клиент", last_name="Отказной", status="refused", advertisement=self.ad
        )
        self.lead_converted = Lead.objects.create(
            first_name="Пётр", last_name="Покупатель", status="converted", advertisement=self.ad
        )
    ...
    def test_user_role_auto_group_assignment(self):
        ...
        self.assertIn(manager_group, user.groups.all(), "Пользователю не привязалась группа 'Managers'!")
    ...
    def test_ads_statistic_sql_logic(self):
        ...
        archived_contract = Contract.objects.create(
            customer=customer,
            service=self.service,
            cost=15000,
            start_date=today,
        ...
        admin = User.objects.create_superuser(username="admin_tester", password="password")
        ...
        self.assertEqual(ad_metrics.leads_count, 2, "Неверный подсчет лидов с учетом отсечения отказа!")
        self.assertEqual(ad_metrics.customers_count, 1, "Неверный подсчет активных клиентов!")
        self.assertEqual(ad_metrics.profit, 25000, "Ошибка в расчете чистой прибыли!")

    def test_role_based_access_control_security(self):
        operator = User.objects.create_user(username="operator_tester", password="password")
        ...
        self.assertEqual(response.status_code, 403, "Дыра в безопасности! Оператор смог открыть страницу статистики!")
    ...
    def test_contract_delete_view_soft_deletes_contract(self):
        ...
        contract = Contract.objects.create(
            customer=customer, service=self.service, cost=5000,
            start_date=today, end_date=today + timedelta(days=30), is_active=True
        )

        admin = User.objects.create_superuser(username="delete_manager", password="password")
        ...
        self.assertFalse(db_contract.is_active,
                         "View удаления физически стерла контракт или не переключила флаг is_active!")
    ...
    def test_lead_conversion_transaction_fail(self):
        ...
        try:
            with transaction.atomic():
                new_customer = Customer.objects.create(lead=self.lead_in_work, is_active=True)
                Contract.objects.create(
                    customer=new_customer, service=self.service, cost=10000,
