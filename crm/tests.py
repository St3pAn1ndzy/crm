...
class CrmBusinessLogicTestCase(TestCase):
    def setUp(self):
        ...
        self.service = Service.objects.create(title="Тестовая услуга",
                                              price=10000, is_active=True)
        ...
        self.lead_in_work = Lead.objects.create(
            first_name="Иван", last_name="ПростоЛид",
            status="new", advertisement=self.ad
        )
        self.lead_refused = Lead.objects.create(
            first_name="Клиент", last_name="Отказной",
            status="refused", advertisement=self.ad
        )
        self.lead_converted = Lead.objects.create(
            first_name="Пётр", last_name="Покупатель",
            status="converted", advertisement=self.ad
        )
    ...
    def test_user_role_auto_group_assignment(self):
        ...
        self.assertIn(manager_group, user.groups.all(), "Пользователю не привязалась "
                                                        "группа 'Managers'!")
    ...
    def test_ads_statistic_sql_logic(self):
        ...
        self.assertEqual(ad_metrics.leads_count, 2, "Неверный подсчет лидов "
                                                    "с учетом отсечения отказа!")
        self.assertEqual(ad_metrics.customers_count, 1, "Неверный подсчет "
                                                        "активных клиентов!")
    ...
    def test_role_based_access_control_security(self):
        ...
        self.assertEqual(
            response.status_code, 403, "Дыра в безопасности! "
                                       "Оператор смог открыть страницу статистики!"
        )
    ...
    def test_contract_delete_view_soft_deletes_contract(self):
        ...
        User.objects.create_superuser(
            username="delete_manager", password="password")
